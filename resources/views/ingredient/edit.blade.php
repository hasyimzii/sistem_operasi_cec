...
@section('title', 'Edit Komposisi')
@section('content')
<div class="row page-titles mx-0" style="background: #343957;">
    <div class="col-sm-6 mt-1 p-md-0">
        <div class="welcome-text">
            <h4 class="text-white">Edit Komposisi</h4>
        </div>
    </div>
    <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
        <a href="{{ route('ingredient.list', $ingredient->product->id) }}">
        ...
        <div class="card-body">
            <div class="basic-form">
                <form action="{{ route('ingredient.update', $ingredient->id) }}" method="post">
                    @csrf
                    <input type="number" name="product_id" value="{{ $ingredient->product->id }}" hidden>
                    <div class="form-group">
                        <label>Nama Barang</label>
                        <input type="text" class="form-control input-default " name="name"
                            placeholder="Tulis nama barang..." value="{{ $ingredient->name }}" required>
                    </div>
                    <div class="form-group">
                        <label>Jumlah</label>
                        <input type="number" class="form-control input-default " name="amount"
                            placeholder="Tulis jumlah barang..." value="{{ $ingredient->amount }}" required>
                    </div>
                    <div class="form-group">
                        <label>Satuan (contoh: kg, liter, pack, dll)</label>
                        <input type="text" class="form-control input-default " name="unit"
                            placeholder="Tulis satuan jumlah barang..." value="{{ $ingredient->unit }}" required>
                    </div><br>
